    ...
    for root, dirs, files in os.walk(docs_dir):
        for file in files:
            ...
            try:
                ...
                # B. Explicit WikiLinks
                wikilinks = re.findall(r'\[\[(.*?)\]\]', content)
                for link in wikilinks:
                    add_ref(link, task_id)

                # C. LLM Extraction (Smart Indexing)
                # Only run if we have an API key and the file is substantial
                if llm and os.getenv("ANTHROPIC_API_KEY") or os.getenv("OPENAI_API_KEY"):
                     # Simple heuristic: don't re-index everything every time (too slow/costly)
                     # For now, we skip this optimization in V1 but note it for V2.
                     pass 
