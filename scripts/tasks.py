...
def compact_task(task_id, summary, output_format="text"):
    ...
    try:
        ...
        # 2a. Generate Summary (if not provided and LLM available)
        final_summary = summary
        if not final_summary:
            if llm:
                print(f"Generating summary for {task_id} using LLM...")
                try:
                    prompt = f"Please provide a concise, high-level summary of the following completed task. Focus on what was accomplished, key decisions, and any important context for future reference.\n\nTask Content:\n{content}"
                    final_summary = llm.complete(prompt, system="You are a helpful technical project manager summarizing completed tasks.")
                except Exception as llm_err:
                    print(f"Warning: LLM generation failed: {llm_err}")
                    final_summary = "Compacted without summary (LLM failure)."
            else:
                final_summary = "Compacted without summary (No LLM client or summary provided)."
...
def main():
    ...
    compact_parser.add_argument("--summary", help="Summary text (optional, auto-generated if omitted)", required=False)
    ...
    if args.command == "create":
        ...
        # Make summary optional if LLM is present, but we need to handle that in the function
